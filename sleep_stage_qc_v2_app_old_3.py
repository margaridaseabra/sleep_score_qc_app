...
with tab_import:
    ...
    if mat_file and safe_mat_keys(mat_file):
        ...
        default_photometry_index = default_key_index(photometry_options, ["ne", "NE", "ach", "ACh"])
        photometry_key = st.selectbox(
            "Optional ACh / fiber photometry variable",
            photometry_options,
            index=default_photometry_index,
            help=(
                "For these recordings, choose `ne`. The app will automatically use "
                "`ne_frequency` as the sampling rate when that variable exists."
            ),
        )

        photometry_freq_value, photometry_freq_key = infer_frequency_value_for_signal(
            mat_file,
            photometry_key,
            keys,
        )
        ...
        photometry_freq_value = None
        photometry_freq_key = None
    ...
    with photometry_import_cols[1]:
        photometry_fs = st.number_input(
            ...
            help=(
                "Automatically filled from the matching frequency variable when available. "
                "For `ne`, this should come from `ne_frequency`."
            ),
        )

    if str(photometry_key).strip():
        if photometry_freq_key is not None and photometry_freq_value is not None:
            st.caption(
                f"Photometry trace selected: `{photometry_key}` | "
                f"sampling-rate variable: `{photometry_freq_key}` = {float(photometry_freq_value):.3f} Hz"
            )
        else:
            st.caption(
                f"Photometry trace selected: `{photometry_key}` | "
                "no matching frequency variable found, so the value above will be used."
            )
