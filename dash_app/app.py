            ...
            html.Div(className="card", children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        dcc.Interval(id="epoch-review-clock", interval=200, n_intervals=0, disabled=True),
                        # This faster clock is client-side only: it moves the
                        # Plotly playhead without re-rendering signal data.
                        dcc.Interval(id="epoch-review-playhead-clock", interval=100, n_intervals=0, disabled=True),
                    ],
                ),
...
@app.callback(
    ...
    Output("window-store", "data", allow_duplicate=True),
    Output("selected-interval-store", "data", allow_duplicate=True),
    Output("qc-graph", "selectedData", allow_duplicate=True),
    Output("selected-interval-label", "children", allow_duplicate=True),
...
def update_selection(selected, fig):
    ...
    dragmode = str((((fig or {}).get("layout") or {}).get("dragmode") or "")).lower()
    if dragmode != "select":
        return no_update, no_update, no_update
...
@app.callback(
    Output("epoch-review-clock", "disabled"),
    Output("epoch-review-playhead-clock", "disabled"),
    Input("selected-interval-store", "data"),
)
def toggle_epoch_review_clocks(selected):
...
def refresh_qc_dissociation_events(n, recording_id, project_root):
    ...
    # Loading a recording should make dissociation events available, but it
    # must not behave as though the user selected event #1.
    return options, None, f"Loaded {len(options)} dissociation events."
