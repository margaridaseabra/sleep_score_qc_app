...
def default_key_index(options, preferred_names, fallback=0):
    """
    Return the selectbox index for the first preferred variable name that exists.
    Matching is case-insensitive but otherwise exact, so eeg -> eeg and emg -> emg.
    """
    options = list(options)
    lower_to_index = {str(opt).lower(): i for i, opt in enumerate(options)}

    for name in preferred_names:
        idx = lower_to_index.get(str(name).lower())
        if idx is not None:
            return idx

    return int(fallback) if options else 0


def read_mat_scalar(mat_file, key):
    """
    Safely read a scalar value from either a normal MATLAB file or an HDF5/v7.3 file.
    Returns None if the key is missing or cannot be converted to a finite float.
    """
    key = str(key).strip()
    if not key:
        return None
...
def infer_frequency_key_for_signal(signal_key, mat_keys):
    """
    Find the most likely sampling-frequency variable for a selected signal.
    For your ACh/FP trace stored as `ne`, this will pick `ne_frequency`.
    """
    ...
    candidates = [
        f"{signal_key}_frequency",
        f"{signal_key}_fs",
        f"{signal_key}_sampling_rate",
        f"{signal_key}_sampling_rate_hz",
        f"{signal_key}Frequency",
        f"{signal_key}Fs",
    ]

    # Common lab-export aliases.
    if signal_key.lower() == "ne":
        candidates = ["ne_frequency", "NE_frequency", "ne_fs", "NE_fs"] + candidates
    elif signal_key.lower() == "eeg":
        candidates = ["eeg_frequency", "EEG_frequency", "eeg_fs", "EEG_fs"] + candidates
    elif signal_key.lower() == "emg":
        candidates = ["emg_frequency", "EMG_frequency", "emg_fs", "EMG_fs"] + candidates

    key_lookup = {str(k).lower(): str(k) for k in mat_keys}
    for candidate in candidates:
        hit = key_lookup.get(str(candidate).lower())
        if hit is not None:
            return hit
...
def infer_frequency_value_for_signal(mat_file, signal_key, mat_keys):
    freq_key = infer_frequency_key_for_signal(signal_key, mat_keys)
    if freq_key is None:
        return None, None
    return read_mat_scalar(mat_file, freq_key), freq_key
...
def robust_z_trace(x, clip=5.0):
    ...
    Useful for photometry because raw fluorescence scale can vary strongly
    across recordings/channels.
    """
...
def save_optional_photometry_from_mat(
    ...
    It saves a selected .mat signal variable, such as `ne`, into the standardized
    recording folder, and stores both the selected signal variable and the selected
    sampling-rate variable in metadata.json. This makes the photometry panel fully
    user-controlled instead of hard-coding `ne_frequency`.
    """
    photometry_key = str(photometry_key).strip()
    sampling_rate_key = str(sampling_rate_key).strip()
    ...
    metadata["photometry_source_variable"] = photometry_key

    if sampling_rate_key:
        metadata["photometry_sampling_rate_variable"] = sampling_rate_key

    if str(photometry_key).strip().lower() == "ne":
        metadata["ne_file"] = output_name
        metadata["ne_label"] = "ACh / fiber photometry (ne)"
        metadata["ne_source_variable"] = photometry_key
        if sampling_rate_key:
            metadata["ne_sampling_rate_variable"] = sampling_rate_key

    if sampling_rate_hz is not None:
        try:
            metadata["photometry_sampling_rate_hz"] = float(sampling_rate_hz)
            if str(photometry_key).strip().lower() == "ne":
                metadata["ne_sampling_rate_hz"] = float(sampling_rate_hz)
        except Exception:
            pass
...
def make_qc_plot(
    ...
    # Safety: Final scoring is visual feedback only: colored shading over raw traces.
    ...
    # ------------------------------------------------------------------
    # Raw EEG/EMG for current visible window
    # ------------------------------------------------------------------
    ...
    if plot_photometry:
        try:
            ...
            phot_plot = robust_z_trace(phot_plot_raw)
    ...
    if show_spectrogram:
        try:
            t_spec_min, f_spec, spec_img, spec_vmin, spec_vmax = make_review_spectrogram_cached(
                str(recording_dir / "eeg.npy"),
                float(fs),
                round(float(start_min), 4),
                round(float(visible_end_min_for_signal), 4),
    ...
    if plot_photometry:
        ...
        subplot_titles.append(f"{phot_label} ({phot_mode}, robust z display)")
    else:
        phot_row = None
    ...
    if plot_photometry:
        fig.add_trace(
            go.Scattergl(
                ...
                mode="lines",
                name=phot_label,
                line=dict(color="#9467bd", width=0.9),
                hovertemplate="time=%{x:.2f} min<br>photometry z=%{y:.2f}<extra></extra>",
            ),
            row=phot_row,
            col=1,
        )
    ...
    # Y-axis behavior:
    fig.update_yaxes(fixedrange=True, row=scoring_row, col=1)
    fig.update_yaxes(fixedrange=False, title_text="EEG", row=eeg_row, col=1)

    if plot_spectrogram:
        fig.update_yaxes(
            fixedrange=True,
            title_text="EEG Hz",
            range=[0, float(spectrogram_fmax)],
            row=spec_row,
            col=1,
        )
    ...
    fig.update_xaxes(title_text="Time from recording start (min)", row=prob_row, col=1)
    ...
    if show_spectrogram and not spec_available and spec_error:
        fig.add_annotation(
            text=f"Spectrogram unavailable: {spec_error}",
            xref="paper",
            yref="paper",
            x=0.99,
            y=0.99,
...
def add_recent_review_shades_to_fig(fig, recording_id):
    ...
    for shade in shades:
        ...
        # Shade raw EEG, optional spectrogram, raw EMG, EMG RMS, and probability panels.
        # Do not shade the scoring-row panel.
        max_row = 6 if "xaxis6" in fig.layout else 5
...
def make_cached_review_base_qc_plot(
    ...
    return make_qc_plot(
        ...
        show_spectrogram=bool(show_spectrogram),
...
def get_cached_review_base_qc_plot(
    recording_dir,
    start_min,
    window_min,
...
# =============================================================================
# WINDOWED EEG SPECTROGRAM
# =============================================================================

@st.cache_data(show_spinner=False, max_entries=128)
def make_review_spectrogram_cached(
    eeg_path_str,
    fs,
    start_min,
    end_min,
    ...
    The cache key includes the EEG file mtime and the selected time window, so the
    spectrogram updates when the source file or visible review chunk changes.
    """
    eeg_path = Path(eeg_path_str)
    eeg = np.load(str(eeg_path), mmap_mode="r")
    ...
    eeg_win = np.asarray(eeg[i0:i1], dtype=np.float32)

    if len(eeg_win) < max(16, int(fs * 2)):
        raise ValueError("Selected EEG window is too short for spectrogram.")

    eeg_win = eeg_win - np.nanmedian(eeg_win)

    nperseg = max(8, int(round(float(window_sec) * fs)))
    nperseg = min(nperseg, len(eeg_win))

    noverlap = int(round(nperseg * float(overlap_fraction)))
    noverlap = min(max(0, noverlap), nperseg - 1)

    nfft = max(nperseg, int(round(fs * 4)))
    ...
    mu = np.nanmean(logp, axis=1, keepdims=True)
    sd = np.nanstd(logp, axis=1, keepdims=True)
    sd[~np.isfinite(sd) | (sd == 0)] = 1.0

    img = (logp - mu) / sd
    img = gaussian_filter(img, sigma=(0.6, 1.2))

    # Keep Plotly responsive if the window is long.
    if img.shape[1] > int(max_time_bins):
        step = int(np.ceil(img.shape[1] / int(max_time_bins)))
        img = img[:, ::step]
        t = t[::step]

    vmin = float(np.nanpercentile(img, 5))
    vmax = float(np.nanpercentile(img, 95))

    t_min = (t + start_s) / 60.0

    return (
        t_min.astype(np.float32),
        f.astype(np.float32),
...
with tab_import:
    ...
    if mat_file and safe_mat_keys(mat_file):
        ...
        eeg_key = st.selectbox(
            "EEG variable",
            keys,
            index=default_key_index(keys, ["eeg", "EEG"]),
        )
        emg_key = st.selectbox(
            "EMG variable",
            keys,
            index=default_key_index(keys, ["emg", "EMG"]),
        )
        ...
        default_photometry_index = default_key_index(
            photometry_options,
            ["ne", "NE", "ach", "ACh", "photometry", "fp"],
        )
        photometry_key = st.selectbox(
            "Optional ACh / fiber photometry trace variable",
            photometry_options,
            index=default_photometry_index,
            help=(
                "Choose the .mat variable containing the photometry trace. "
                "For your current files this should usually be `ne`."
            ),
        )

        # The frequency should also be chosen by the user, exactly like the
        # signal variable. We still infer a good default so `ne` preselects
        # `ne_frequency` when present, but the user remains in control.
        inferred_photometry_freq_key = infer_frequency_key_for_signal(
            photometry_key,
            keys,
        )
        photometry_frequency_options = [""] + keys
        default_photometry_freq_index = (
            photometry_frequency_options.index(inferred_photometry_freq_key)
            if inferred_photometry_freq_key in photometry_frequency_options
            else default_key_index(
                photometry_frequency_options,
                ["ne_frequency", "NE_frequency", "ne_fs", "NE_fs"],
            )
        )

        photometry_freq_key = st.selectbox(
            "Optional ACh / fiber photometry frequency variable",
            photometry_frequency_options,
            index=default_photometry_freq_index,
            help=(
                "Choose the .mat variable that contains the sampling rate for the "
                "photometry trace. For your current files this should usually be "
                "`ne_frequency`."
            ),
        )

        photometry_freq_value = read_mat_scalar(mat_file, photometry_freq_key)
    else:
        keys = []
        ...
        photometry_key = st.text_input("Optional ACh / fiber photometry trace variable", "ne")
        photometry_freq_key = st.text_input("Optional ACh / fiber photometry frequency variable", "ne_frequency")
        photometry_freq_value = read_mat_scalar(mat_file, photometry_freq_key) if mat_file else None

    epoch_sec = st.number_input("Epoch length, seconds", min_value=0.1, value=1.0, step=0.5)

    default_photometry_output = "ne.npy" if str(photometry_key).strip().lower() == "ne" else "ach.npy"
    default_photometry_fs = float(photometry_freq_value) if photometry_freq_value is not None else float(fs)
    ...
    with photometry_import_cols[0]:
        photometry_output_name = st.text_input(
            ...
            value=default_photometry_output,
            key=f"photometry_output_name_{str(photometry_key)}",
            help=(
                "Saved inside the recording folder. For your files, `ne.npy` is the clearest name. "
                "The app also auto-detects ne.npy, ach.npy, photometry.npy, etc."
            ),
        )
    with photometry_import_cols[1]:
        photometry_fs = st.number_input(
            ...
            min_value=1.0,
            value=default_photometry_fs,
            step=1.0,
            key=f"photometry_fs_{str(photometry_key)}_{str(photometry_freq_key)}",
            help=(
                "This is filled from the selected frequency variable above when possible. "
                "You can still edit it manually if the .mat variable is missing or wrong."
            ),
        )

    if str(photometry_key).strip():
        if str(photometry_freq_key).strip() and photometry_freq_value is not None:
            st.caption(
                f"Photometry trace variable: `{photometry_key}` | "
                f"frequency variable: `{photometry_freq_key}` = {float(photometry_freq_value):.3f} Hz"
            )
        elif str(photometry_freq_key).strip():
            st.caption(
                f"Photometry trace variable: `{photometry_key}` | "
                f"frequency variable `{photometry_freq_key}` could not be read, "
                "so the numeric value above will be used."
            )
        else:
            st.caption(
                f"Photometry trace variable: `{photometry_key}` | "
                "no frequency variable selected, so the numeric value above will be used."
            )
                ...
                if str(photometry_key).strip():
                    try:
                        phot_out = save_optional_photometry_from_mat(
                            ...
                            sampling_rate_key=photometry_freq_key,
                        )
...
with tab_review:
    ...
    st.markdown(
        ...
        <div style="display:flex; gap:18px; flex-wrap:wrap; align-items:center; margin-bottom:12px;">
          ...
          <span><span style="display:inline-block;width:24px;border-top:2px solid #9467bd;margin-right:5px;vertical-align:middle;"></span>ACh / fiber photometry robust z</span>
        </div>
        """,
    ...
    # --------------------------------------------------------------
    # Spectrogram controls for the Review/Edit plot
    # --------------------------------------------------------------
    ...
    with spec_c1:
        show_review_spectrogram = st.checkbox(
            ...
            value=True,
            key="review_show_eeg_spectrogram",
            help="Adds a time-frequency view for the currently visible review window.",
        )

    with spec_c2:
        review_spectrogram_fmax = st.number_input(
            "Spectrogram max Hz",
            min_value=5.0,
            max_value=80.0,
            value=20.0,
            step=5.0,
            key="review_spectrogram_fmax",
        )
    ...
    try:
        ...
        if interaction_event is not None and isinstance(interaction_event, dict):
            ...
            if event_type == "relayout":
                ...
                if sel_x0 is not None and sel_x1 is not None:
                    ...
                    if sel_x1 > sel_x0:
                        current_sel0 = float(st.session_state.get("selected_label_start_min", selected_period[0]))
                        current_sel1 = float(st.session_state.get("selected_label_end_min", selected_period[1]))

                        selection_changed = (
                            abs(sel_x0 - current_sel0) > plotly_event_tol_min
                            or abs(sel_x1 - current_sel1) > plotly_event_tol_min
                        )

                        event_signature = f"selection:{sel_x0:.3f}:{sel_x1:.3f}"
                        already_processed = (
                            st.session_state.get(plotly_event_memory_key) == event_signature
                        )

                        if selection_changed and not already_processed:
                            st.session_state[plotly_event_memory_key] = event_signature
