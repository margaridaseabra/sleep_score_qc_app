...
def make_qc_plot(
    ...
    if plot_photometry:
        ...
        subplot_titles.append(f"{phot_label} ({phot_mode}, raw display)")
    else:
        phot_row = None
...
with tab_review:
    ...
    st.markdown(
        ...
        <div style="display:flex; gap:18px; flex-wrap:wrap; align-items:center; margin-bottom:12px;">
          ...
          <span><span style="display:inline-block;width:24px;border-top:2px solid #9467bd;margin-right:5px;vertical-align:middle;"></span>ACh / fiber photometry raw</span>
        </div>
        """,
    ...
    with perf_c2:
        if fast_review_plotly_mode:
            st.caption("Fast mode: ~60k points per raw trace, ~1200 spectrogram time bins, lower spectrogram overlap.")
        else:
            st.caption("High-detail mode: more raw points and denser spectrogram; slower when changing windows.")

    review_max_raw_points = 60000 if fast_review_plotly_mode else 200000
